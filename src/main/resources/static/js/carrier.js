...
function toAdd() {
    //清除输入框内容
    $("#add_carrierName").val("");//名称
    $("#add_address").val("");//描述
    $("#add_carrierNumber").val("");//编号
    $("#add_contact").val("");
    $("#add_phone").val("");
    //打开模态框
    $("#addModal").modal("show");
    ...
    $("#addModal").on("shown.bs.modal", function () {
        $("#add_carrierName").focus();
    })
}

function add() {
    //获取参数
    var carrierName = $("#add_carrierName").val();//角色名称
    var carrierNumber = $("#add_carrierNumber").val();//编号
    var address=$("#add_address").val();//描述
    ...
    if (carrierName == "") {
        ...
        $.ajax({
            url: 'carrierAdd',
            type: 'post',
            data: {'carrierName': carrierName, 'carrierNumber': carrierNumber, 'contact': contact,'phone':phone,'address':address},
...
function toUpdate(a) {
    ...
    var carrierName = td[1].innerHTML;//2.名称
    var carrierNumber=td[2].innerHTML;//编号
    var address = td[5].innerHTML;//3.描述
    var contact=td[3].innerHTML;
    var phone=td[4].innerHTML;
    //把参数放入输入框
    $("#update_id").val(id);
    $("#update_carrierName").val(carrierName);
    $("#update_carrierNumber").val(carrierNumber);
...
function update() {
    ...
    var carrierName = $("#update_carrierName").val();//名称
    var carrierNumber = $("#update_carrierNumber").val();//编号
    ...
    if (carrierName == "") {
        ...
        $.ajax({
            url: 'carrierUpdate',
            type: 'post',
            data: {'id': id, 'carrierName': carrierName, 'carrierNumber': carrierNumber, 'contact': contact,'phone':phone,'address':address},
            success: function (data) {
                if (data.code==0) {
                    var currentPage=$("#span_currentPage").html();
                    findByLimit(currentPage);
...
function findByLimit(currentPage) {
    ...
    $("#div_loading").css("display", "block");
    //获取参数
    var carrierName=$("#input_carrierName").val();
    var carrierNumber=$("#input_carrierNumber").val();
    //先清除旧数据
    $("#table_data").html("");
    //后台查询
    $.ajax({
            ...
            if (data.data != null) {
                ...
                for (var i = 0; i < record.length; i++) {
                    var str = "<tr><td style='display: none'>" + record[i].id +
                        "</td><td>" + record[i].carriername +
                        "</td><td>" + record[i].carriernumber +
