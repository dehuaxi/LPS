...
//查看运输单详情
function toDetail(btn){
    $("#table_detail").html("");
    var billNumber=$(btn).parent().parent().find("td:eq(0)").text();
    $.ajax({
...
function toPrint(btn){
    var billNumber=$(btn).parent().parent().find("td:eq(0)").text();
    var tips=confirm("确认打印运输单？");
    if(tips){
        window.open("transportBillCacheBillPrint?billNumber="+billNumber);
    }
}

//确认到达目的地
function toReceive(btn){
    var billNumber=$(btn).parent().parent().find("td:eq(0)").text();
    var tips=confirm(billNumber+"确认到达目的地？");
    if(tips){
        $("#div_loading").css("display","block");
        $.ajax({
            url: 'transportBillRecordAdd',
            type: 'post',
            dataType:'json',
            data:{'billNumber':billNumber},
            success: function (data) {
        ...
        success: function (data) {
            if(data.code==0){
                ...
                if (data.data != null) {
                    ...
                    var deleteBtn="";
                    if(detailData){
                        deleteBtn="<button type='button' class='btn btn-danger btn-xs' onclick='toDetail(this)'>查看详情</button>";
                    }
                    var updateBtn="";
                    if(printData){
                        updateBtn=" <button type='button' class='btn btn-warning btn-xs' onclick='toPrint(this)'>打印运输单</button>";
                    }
                    var addBtn="";
                    if(addData){
                        addBtn=" <button type='button' class='btn btn-primary btn-xs' onclick='toReceive(this)'>确认到达</button>";
                    }
                    var td="";
                    if(detailData==false&&printData==false&&addData==false){
                        td="";
                    }else {
                        td="<td>"+updateBtn+deleteBtn+addBtn+"</td>";
                    }
                    for (var i = 0; i < record.length; i++) {
