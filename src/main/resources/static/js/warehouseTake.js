...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.code==0){
                for(var i=0;i<data.data.length;i++){
                    $("#select_startName").append("<option>"+data.data[i].warehousename+"</option>");
                    $("#select_endName").append("<option>"+data.data[i].warehousename+"</option>");
                }
            } else {
                alert(data.msg);
            }
        ...
        error:function(jqXHR, textStatus, errorThrown){
            ...
            alert("加载出发地、目的地中转仓信息失败：" + status + "  " + text);
        }
    });
...
//绑定车辆出库
function toOutAdd(btn){
    var billNumber=$(btn).parent().parent().find("td:eq(0)").text();
    window.open("toWarehouseOutAdd?billNumber="+billNumber);
}
//------------------------------------分页查询收货记录-------------------------------
//时间格式化
        ...
        success: function (data) {
            if(data.code==0){
                ...
                if (data.data != null) {
                    ...
                    var td="";
                    if(deleteData){
                        td="<td><button type='button' class='btn btn-warning btn-xs' onclick='toOutAdd(this)'>绑定车辆出库</button></td>";
                    }
                    for (var i = 0; i < record.length; i++) {
                        var str= "<tr title='点击方案编号查看详情'><td><a href='#' onclick='toDetail(this)'>" + record[i].billnumber +"</a></td>"+
