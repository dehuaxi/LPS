...
$(document).ready(function () {
    laydate.render({
        elem: '#input_uploadDate',//指定元素
        type: 'date'//日期
    });
    laydate.render({
        elem: '#input_receiveDateStart',//指定元素
        type: 'date'//日期
    });
    laydate.render({
        elem: '#input_receiveDateEnd',//指定元素
        type: 'date'//日期
    });
...
function toBind(btn){
    ...
    $("#bindModal_billNumber").val("");
    $("#bindModal_id").val(id);
    $("#bindModal").modal("show");
    $("#bindModal").on("shown.bs.modal",function (){
        $("#bindModal_billNumber").focus();
    })
}
function bind(){
    var id=$("#bindModal_id").val();
    var billNumber=$("#bindModal_billNumber").val();
    $("#div_loading").css("display", "block");
    $.ajax({
        url: 'geelyBillRecordBind',
        type: 'post',
        data: {
            'id':id,
...
function toDownload() {
    ...
    var needBind=$("#select_needBind").val();
    var bindBillNumber=$("#input_bindBillNumber").val();
    var uploadDate=$("#input_uploadDate").val();
    var receiveDateStart=$("#input_receiveDateStart").val();
    var receiveDateEnd=$("#input_receiveDateEnd").val();
    if(receiveDateStart==""){
        alert("必须选择回执日期(开始)")
    }else if(receiveDateEnd==""){
        alert("必须选择回执日期(结束)")
    }else if(datedifference(receiveDateStart, receiveDateEnd)>=31){
        alert("回执日期(开始)和回执日期(结束)之间不能超过31天")
    }else {
        window.open("billRecordDownload?goodCode="+goodCode+"&goodName="+goodName+"&supplierCode="+supplierCode+"&supplierName="+supplierName+"&billNumber="+billNumber+"&batch="+batch+"&needBind="+needBind+"&bindBillNumber="+bindBillNumber+"&uploadDate="+uploadDate+"&receiveDateStart="+receiveDateStart+"&receiveDateEnd="+receiveDateEnd);
    }
}
            ...
            if(data.code==0){
                ...
                if (data.data != null) {
                    ...
                    for (var i = 0; i < record.length; i++) {
                        ...
                        var bindBtn="";
                        if(bindData){
                            bindBtn="<td><button type='button' class='btn btn-xs btn-warning' onclick='toBind(this)'>绑定补充单</button></td>";
                        }
                        var str = "<tr style='color:"+fontColor+";'><td style='display: none'>" + record[i].id + "</td>"+
                            "<td>" + record[i].billnumber +"</td>"+
                            "<td>" + goodcode +"</td>"+
                            "<td>" + goodname +"</td>"+
                            "<td>" + suppliercode +"</td>"+
                            "<td>" + suppliername +"</td>"+
                            ...
                            "<td>" + record[i].needbind +"</td>"+
                            "<td>" + record[i].bindbillnumber +"</td>"+
                            "<td>" + record[i].uploadtime +"</td>"+
                            "<td>" + timeFormat(record[i].receivetime) +"</td>" +
                            "<td>" + record[i].remarks +"</td>"+bindBtn+"</tr>";
                        $("#table_data").append(str);
                    }
                    //6、需要绑定数量显示
                    $("#needBindCount").html(datas.needBindCount);
                } else {
                    //隐藏分页信息
